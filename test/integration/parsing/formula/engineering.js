...
describe('.parse() engineering formulas', () => {
  ...
  it('BESSELI', () => {
    expect(parser.parse('BESSELI()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('BESSELI(1.4)')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('BESSELI(1.4, 1)')).toBeMatchCloseTo({error: null, result: 0.8860919793963105});
  });

  it('BESSELJ', () => {
    expect(parser.parse('BESSELJ()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('BESSELJ(1.4)')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('BESSELJ(1.4, 1)')).toBeMatchCloseTo({error: null, result: 0.5419477138848564});
  });

  it('BESSELK', () => {
    expect(parser.parse('BESSELK()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('BESSELK(1.4)')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('BESSELK(1.4, 1)')).toMatchObject({error: null, result: 0.32083590550458985});
  });

  it('BESSELY', () => {
    expect(parser.parse('BESSELY()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('BESSELY(1.4)')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('BESSELY(1.4, 1)')).toMatchObject({error: null, result: -0.47914697411134044});
  });

  it('BIN2DEC', () => {
    expect(parser.parse('BIN2DEC()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('BIN2HEX', () => {
    expect(parser.parse('BIN2HEX()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('BIN2OCT', () => {
    expect(parser.parse('BIN2OCT()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('BITAND', () => {
    expect(parser.parse('BITAND()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('BITAND(2)')).toMatchObject({error: '#N/A', result: null});
  ...
  it('BITLSHIFT', () => {
    expect(parser.parse('BITLSHIFT()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('BITLSHIFT(2)')).toMatchObject({error: '#N/A', result: null});
  ...
  it('BITOR', () => {
    expect(parser.parse('BITOR()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('BITOR(2)')).toMatchObject({error: '#N/A', result: null});
  ...
  it('BITRSHIFT', () => {
    expect(parser.parse('BITRSHIFT()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('BITRSHIFT(2)')).toMatchObject({error: '#N/A', result: null});
  ...
  it('BITXOR', () => {
    expect(parser.parse('BITXOR()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('BITXOR(2)')).toMatchObject({error: '#N/A', result: null});
  ...
  it('COMPLEX', () => {
    expect(parser.parse('COMPLEX()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('CONVERT', () => {
    expect(parser.parse('CONVERT()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('CONVERT(1)')).toMatchObject({error: '#N/A', result: null});
  ...
  it('DEC2BIN', () => {
    expect(parser.parse('DEC2BIN()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('DEC2HEX', () => {
    expect(parser.parse('DEC2HEX()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('DEC2OCT', () => {
    expect(parser.parse('DEC2OCT()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('DELTA', () => {
    expect(parser.parse('DELTA()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('ERF', () => {
    expect(parser.parse('ERF()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('ERFC', () => {
    expect(parser.parse('ERFC()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('GESTEP', () => {
    expect(parser.parse('GESTEP()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('HEX2BIN', () => {
    expect(parser.parse('HEX2BIN()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('HEX2DEC', () => {
    expect(parser.parse('HEX2DEC()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('HEX2OCT', () => {
    expect(parser.parse('HEX2OCT()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('IMABS', () => {
    expect(parser.parse('IMABS()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('IMABS("5+12i")')).toMatchObject({error: null, result: 13});
  });

  it('IMAGINARY', () => {
    expect(parser.parse('IMAGINARY()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('IMARGUMENT', () => {
    expect(parser.parse('IMARGUMENT()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('IMARGUMENT(1)')).toMatchObject({error: null, result: 0});
  ...
  it('IMCONJUGATE', () => {
    expect(parser.parse('IMCONJUGATE()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('IMCOS', () => {
    expect(parser.parse('IMCOS()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('IMCOSH', () => {
    expect(parser.parse('IMCOSH()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('IMCOSH("3+4i")')).toMatchObject({error: null, result: '-6.580663040551157-7.581552742746545i'});
  });

  it('IMCOT', () => {
    expect(parser.parse('IMCOT()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('IMREAL', () => {
    expect(parser.parse('IMREAL()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('IMREAL("3+4i")')).toMatchObject({error: null, result: 3});
  });

  it('IMSEC', () => {
    expect(parser.parse('IMSEC()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('IMSIN', () => {
    expect(parser.parse('IMSIN()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('IMSINH', () => {
    expect(parser.parse('IMSINH()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('IMSINH("3+4i")')).toMatchObject({error: null, result: '-6.5481200409110025-7.61923172032141i'});
  });

  it('IMSQRT', () => {
    expect(parser.parse('IMSQRT()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('IMSQRT("3+4i")')).toMatchObject({error: null, result: '2+i'});
  });

  it('IMSUB', () => {
    expect(parser.parse('IMSUB()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('IMSUB("3+4i")')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('IMSUB("3+4i", "2+3i")')).toMatchObject({error: null, result: '1+i'});
  });

  it('IMSUM', () => {
    expect(parser.parse('IMSUM()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('IMTAN', () => {
    expect(parser.parse('IMTAN()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('OCT2BIN', () => {
    expect(parser.parse('OCT2BIN()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('OCT2DEC', () => {
    expect(parser.parse('OCT2DEC()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('OCT2HEX', () => {
    expect(parser.parse('OCT2HEX()')).toMatchObject({error: '#N/A', result: null});
