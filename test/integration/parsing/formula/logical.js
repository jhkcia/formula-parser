...
describe('.parse() logical formulas', () => {
  ...
  it('AND', () => {
    expect(parser.parse('AND()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('IF', () => {
    expect(parser.parse('IF()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('NOT', () => {
    expect(parser.parse('NOT()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('OR', () => {
    expect(parser.parse('OR()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('XOR', () => {
    expect(parser.parse('XOR()')).toMatchObject({error: '#N/A', result: null});
  ...
  it('SWITCH', () => {
    expect(parser.parse('SWITCH()')).toMatchObject({error: '#N/A', result: null});
    expect(parser.parse('SWITCH(7, "foo")')).toMatchObject({error: '#N/A', result: null});
